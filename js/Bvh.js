...
BVH.Reader.prototype = {
        ...
        if (this.type === 'bvh') { // direct from file
            xhr.onreadystatechange = function () {
                if (this.readyState == 4) {
                    _this.parseData(this.responseText.split(/\s+/g));
                }
            };
    ...
    addSkeleton: function (n) {
        ...
        var result = this.bones.map(function (bone) {
            return {name: bone.name, quat: bone.quaternion, euler: bone.rotation}
        })
        console.log(result)
    ...
    updateSkeleton: function () {
        ...
        let result = this.bones.map(function (bone) {
            let euler = bone.rotation;
            return {
                name: bone.name,
                quat: bone.quaternion,
                euler: euler,
    ...
    autoDetectRotation: function (Obj, Axe, Angle) {
        ...
        let logFlag = Obj.name === 'Hips';
        if (logFlag) console.log(Obj.name, Axe, Angle, angle)
    ...
    reset: function () {
        ...
        for (var i = 0, len = ref.length; i < len; i++) {
            ...
            var e = new THREE.Euler(0, 0, 0, 'XYZ');
            ch.node.setRotationFromEuler(e)
            // ch.node.quaternion.set(0, 0, 0, 1);
            console.log(ch.node)
            ch.node.updateMatrixWorld();
        }
